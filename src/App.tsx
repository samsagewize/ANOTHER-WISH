...
import { fetchFeeRates, fetchBalance, toB64, loadSatsConnect, DEV_ADDR, REG_FEE, fetchTxStatus } from './services/bitcoinService';
import { db, auth, collection, doc, setDoc, updateDoc, onSnapshot, query, orderBy, limit, OperationType, handleFirestoreError } from './firebase';
...
export default function App() {
  ...
  // Poll for status updates for pending inscriptions
  useEffect(() => {
          ...
          if (newStatus !== ins.status) {
            ...
            if (ins.registered) {
              try {
                ...
                // Registry doc might not exist yet or at all
              }
            }
          }
        } catch (e) {
    ...
    const timer = setInterval(checkStatuses, 60000); // Check every minute
    checkStatuses();
          ...
          {!wallet ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2 bg-black/40 border border-[#3a2808] rounded-lg px-3 py-2">
                ...
                <button onClick={disconnectWallet} className="text-[#7a5a25] hover:text-[#cc5050] transition-colors">
                  <LogOut size={14} />
                </button>
              </div>
            </div>
            ...
            <motion.main key="ui" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 min-h-[calc(100vh-48px)] bg-[radial-gradient(ellipse_at_50%_0%,#140e04_0%,#070401_70%)]">
                      ...
                      <div className="grid grid-cols-4 gap-2 mb-3">
                        {(['slow', 'med', 'fast', 'custom'] as FeeTier[]).map((tier) => (
                          <button key={tier} onClick={() => setSelRate(tier)} className={`bg-black/30 border rounded-lg p-2 transition-all text-center ${selRate === tier ? 'border-[#c9a040] bg-[rgba(245,200,66,0.07)]' : 'border-[#2a1808] hover:border-[#5a3a18]'}`}>
                            ...
                            <span className="text-[0.76rem] text-[#c9a040] font-medium">{tier === 'custom' ? customRate : feeRates[tier]} s/vB</span>
                          </button>
                        ))}
                      </div>
                      {selRate === 'custom' && (
                        <div className="flex items-center gap-3 bg-black/40 border border-[#2a1808] rounded-lg p-3 animate-in fade-in slide-in-from-top-2">
                          <span className="font-cinzel text-[0.65rem] text-[#7a5a25] uppercase">Sats/vB:</span>
                          <input 
                            type="number" 
                            value={customRate} 
                            onChange={(e) => setCustomRate(Math.max(1, parseInt(e.target.value) || 1))}
                            className="flex-1 bg-transparent border-none outline-none text-[#f5c842] font-mono text-sm"
                          />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-3 mb-6">
                      {!wallet ? (
                        ...
                        <div className="space-y-2">
                          ...
                          <button onClick={disconnectWallet} className="text-[0.63rem] text-[#6a3a18] font-cinzel tracking-widest uppercase underline block mx-auto">Disconnect</button>
                        </div>
                      )}
                    </div>
                            ...
                            <div className="aspect-square w-full bg-black/40 overflow-hidden relative">
                              ...
                              <div className="absolute top-2 right-2">
                                {ins.status === 'pending' || !ins.status ? (
                                  <span className="bg-[#f5c842] text-black text-[0.45rem] px-1.5 py-0.5 rounded font-bold uppercase animate-pulse">Pending</span>
                                ) : ins.status === 'confirmed' ? (
                                  <span className="bg-[#55d070] text-black text-[0.45rem] px-1.5 py-0.5 rounded font-bold uppercase">Confirmed</span>
                                ) : (
                                  <span className="bg-[#cc5050] text-white text-[0.45rem] px-1.5 py-0.5 rounded font-bold uppercase">Failed</span>
                                )}
                              </div>
                            </div>
              ...
              <footer className="text-center py-8 border-t border-[rgba(245,200,66,0.06)]">
                <div className="font-cinzel text-[0.62rem] text-[#3a2808] tracking-[0.1em] uppercase leading-loose">
                  ...
                  <a href="https://x.com/SamSageWize" target="_blank" className="text-[#c9a040] hover:text-[#f5c842] transition-colors font-bold">Founder: @SamSageWize</a><br />
