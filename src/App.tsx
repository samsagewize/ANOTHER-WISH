...
export default function App() {
  ...
  const handleImport = async () => {
    if (!importTxid || !wallet) return;
    setStatus({ type: 'info', msg: 'Importing inscription...' });
    try {
      // We don't have an indexer, so we'll just create a placeholder entry
      // and let the content renderer handle the rest via ordinals.com
      const newIns: Inscription = {
        wish: `Imported Inscription`,
        contentType: 'image/png', // Default to image, renderer will try to load it
        wishTxid: importTxid.trim(),
        regTxid: null,
        address: wallet.ordAddr,
        timestamp: Date.now(),
        registered: false,
      ...
      await setDoc(doc(db, 'inscriptions', newIns.wishTxid), newIns);
      setStatus({ type: 'ok', msg: 'Inscription imported to the well!' });
      setShowImport(false);
      setImportTxid('');
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-black text-[#e8d5a3] font-lora">
                      ...
                      {inscriptions.length === 0 ? (
                        ...
                        inscriptions.sort((a, b) => b.timestamp - a.timestamp).map((item, idx) => (
                          <div key={idx} className={`bg-gradient-to-br from-[#130e06] to-[#0d0804] border border-[#2a1a08] rounded-xl p-5 relative overflow-hidden group hover:border-[#f5c842]/30 transition-all ${wallet?.ordAddr === item.address ? 'border-l-2 border-l-[#f5c842]' : ''}`}>
                            ...
                            <div className="relative aspect-square w-full bg-[#0a0602] rounded-xl overflow-hidden border border-[#2a1a08] mb-4 group-hover:border-[#f5c842]/20 transition-all">
                              {item.contentType.startsWith('image/') ? (
                                <img 
                                  src={`https://ordinals.com/content/${item.wishTxid}i0`} 
                                ...
                                <div className="w-full h-full p-6 flex items-center justify-center text-center">
                                  <p className="text-[#e8d5a3] italic text-lg font-lora">"{item.wish}"</p>
                                </div>
                              )}
                              <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-60" />
                              <div className="absolute bottom-3 left-3 right-3">
                                <p className="text-[0.65rem] text-[#f5c842] font-cinzel tracking-widest uppercase opacity-0 group-hover:opacity-100 transition-opacity">View Content ⟡</p>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between border-t border-[#2a1a08] pt-3 mt-3">
                              <div className="flex items-center gap-3">
                                <a href={`https://mempool.space/tx/${item.wishTxid}`} target="_blank" className="text-[0.62rem] font-cinzel text-[#7a5a25] hover:text-[#f5c842] transition-colors flex items-center gap-1 uppercase tracking-widest">
                                  Mempool <ExternalLink size={10} />
                                </a>
                                <a href={`https://ord.io/${item.wishTxid}i0`} target="_blank" className="text-[0.62rem] font-cinzel text-[#7a5a25] hover:text-[#f5c842] transition-colors flex items-center gap-1 uppercase tracking-widest">
                                  Ord.io <ExternalLink size={10} />
                                </a>
                              </div>
                              <div className="flex items-center gap-2">
                                <div className="w-5 h-5 rounded-full bg-[#1a1208] border border-[#3a2808] flex items-center justify-center text-[0.55rem] text-[#7a5a25]">₿</div>
                                <span className="text-[0.6rem] font-mono text-[#4a3018]">{item.wishTxid.slice(0, 6)}...{item.wishTxid.slice(-6)}</span>
                              </div>
                            </div>
                          </div>
                        ))
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button 
                        ...
                        className="text-[0.6rem] font-cinzel text-[#c9a040] hover:text-[#f5c842] transition-colors flex items-center gap-1 uppercase tracking-widest border border-[#c9a040]/30 rounded px-2 py-1 bg-[#c9a040]/5"
                      >
                        <Upload size={10} /> Import TXID
                      </button>
                    </div>
                    {inscriptions.filter(ins => ins.address === wallet?.ordAddr).length === 0 ? (
                      ...
                      inscriptions.filter(ins => ins.address === wallet?.ordAddr).map((ins, i) => (
                        <div key={i} className="bg-gradient-to-br from-[#130e06] to-[#0d0804] border border-[#2a1a08] rounded-xl p-4 relative overflow-hidden group hover:border-[#3a2a12] transition-all">
                          ...
                          <div className="relative aspect-video w-full bg-black/40 rounded-lg overflow-hidden border border-[#2a1a08] mb-3 group-hover:border-[#3a2a12] transition-all">
                            {ins.contentType.startsWith('image/') ? (
                              <img 
                                src={`https://ordinals.com/content/${ins.wishTxid}i0`} 
                                alt="Inscription content" 
                                className="w-full h-full object-contain"
                                referrerPolicy="no-referrer"
                              />
                              ...
                              <div className="w-full h-full p-3 flex items-center justify-center text-center">
                                <p className="text-[0.75rem] text-[#d4a040] italic leading-relaxed">"{ins.wish}"</p>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="flex flex-wrap gap-x-4 gap-y-2 items-center text-[0.65rem] border-t border-[#2a1a08] pt-3 mt-3">
                            <div className="flex items-center gap-2">
                              <span className="font-cinzel text-[0.58rem] text-[#3a2408] tracking-widest uppercase">Track:</span>
                              <a href={`https://mempool.space/tx/${ins.wishTxid}`} target="_blank" className="text-[#c9a040] hover:underline flex items-center gap-1">
                                Mempool <ExternalLink size={10} />
                              </a>
                              <a href={`https://ord.io/${ins.wishTxid}i0`} target="_blank" className="text-[#c9a040] hover:underline flex items-center gap-1">
                                Ord.io <ExternalLink size={10} />
                              </a>
                              <a href={`https://ordinals.com/inscription/${ins.wishTxid}i0`} target="_blank" className="text-[#c9a040] hover:underline flex items-center gap-1">
                                Ordinals.com <ExternalLink size={10} />
                              </a>
                            </div>
      ...
      <AnimatePresence>
        {showImport && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[10000] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-[#1a120a] border border-[#f5c842]/30 rounded-2xl p-6 max-w-md w-full shadow-2xl">
              <h3 className="font-cinzel text-lg text-[#f5c842] tracking-widest uppercase mb-4">Import Inscription</h3>
              <p className="text-[0.8rem] text-[#7a5a25] mb-4 leading-relaxed">
                If your wish was cast but isn't showing up, you can manually import it using the transaction ID (TXID).
              </p>
              <div className="mb-6">
                <label className="block font-cinzel text-[0.65rem] text-[#7a5a25] tracking-widest uppercase mb-2">Transaction ID (TXID)</label>
                <input 
                  type="text" 
                  value={importTxid}
                  onChange={(e) => setImportTxid(e.target.value)}
                  placeholder="Enter 64-character TXID..."
                  className="w-full bg-black/50 border border-[#2a1808] rounded-lg text-[#e8d5a3] font-mono text-[0.75rem] p-3 outline-none focus:border-[#c9a040] transition-colors"
                />
              </div>
              <div className="flex gap-3">
                <button onClick={() => setShowImport(false)} className="flex-1 py-3 rounded-xl border border-[#3a2808] text-[#7a5a25] font-cinzel text-[0.75rem] tracking-widest uppercase hover:bg-white/5 transition-all">Cancel</button>
                <button onClick={handleImport} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-[#c9a040] to-[#f5c842] text-black font-cinzel font-bold text-[0.75rem] tracking-widest uppercase hover:opacity-90 transition-all shadow-[0_0_20px_rgba(201,160,64,0.3)]">Import</button>
              </div>
            </motion.div>
          </motion.div>
        )}
