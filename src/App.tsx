              ...
              <div className="max-w-[560px] mx-auto px-5 py-8 pb-20">
                <div className="flex gap-1 bg-black/35 border border-[#221508] rounded-xl p-1 mb-6">
                  ...
                  <button onClick={() => setTab('inscriptions')} className={`flex-1 flex items-center justify-center gap-2 font-cinzel text-[0.68rem] font-semibold tracking-widest uppercase py-2 rounded-lg transition-all ${tab === 'inscriptions' ? 'text-[#f5c842] bg-[rgba(245,200,66,0.1)] border border-[rgba(245,200,66,0.22)]' : 'text-[#7a5a25] hover:text-[#c9a040]'}`}>
                    <User size={14} /> Profile <span className="bg-[rgba(245,200,66,0.15)] border border-[rgba(245,200,66,0.28)] rounded-full text-[0.58rem] px-1.5 min-w-[18px] text-center">{inscriptions.length}</span>
                  </button>
                </div>
                ...
                {tab === 'gallery' && (
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {/* Combined Feed: Global + Local */}
                      {[
                        ...[
                          { wish: "May the blocks be full and the fees be low.", timestamp: 1773860000000, wishTxid: "00be645b...d9f1", registered: true, global: true },
                          { wish: "To the moon and beyond, forever on-chain.", timestamp: 1773773600000, wishTxid: "a1b2c3d4...e5f6", registered: true, global: true },
                          { wish: "A digital artifact for my grandchildren.", timestamp: 1773687200000, wishTxid: "f6e5d4c3...b2a1", registered: true, global: true },
                        ],
                        ...inscriptions.map(ins => ({ ...ins, global: false }))
                      ].sort((a, b) => b.timestamp - a.timestamp).map((item, idx) => (
                        <div key={idx} className={`bg-gradient-to-br from-[#130e06] to-[#0d0804] border border-[#2a1a08] rounded-xl p-5 relative overflow-hidden group hover:border-[#f5c842]/30 transition-all ${!item.global ? 'border-l-2 border-l-[#f5c842]' : ''}`}>
                          ...
                          <div className="flex justify-between items-start mb-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`text-[0.55rem] px-2 py-0.5 rounded uppercase font-cinzel tracking-tighter ${item.global ? 'bg-[rgba(245,200,66,0.05)] border border-[#f5c842]/20 text-[#f5c842]' : 'bg-[rgba(80,168,96,0.1)] border border-[#50a860]/30 text-[#50a860]'}`}>
                                {item.global ? 'Verified Inscription' : 'Your Wish'}
                              </span>
                            </div>
                          </div>
