...
export async function fetchTxStatus(txid: string): Promise<'pending' | 'confirmed' | 'failed'> {
  try {
    const r = await fetch(`${MEMPOOL}/tx/${txid}`);
    if (r.ok) {
      const d = await r.json();
      return d.status?.confirmed ? 'confirmed' : 'pending';
    }
    // If 404, it might be too new or failed
